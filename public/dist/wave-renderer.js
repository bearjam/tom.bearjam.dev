...
const range$1 = ({ min, max, step, id, oninput, value }) => callbagHtml.div([
  callbagHtml.input({
    id,
    type: "range",
    min,
...
class WaveRenderer extends HTMLElement {
  ...
  constructor() {
    ...
    const canvasCtx = callbagHtml.canvas().getContext("2d");
    ...
    const waveData =
      fromEvent$1(sourceElement, "waveData").pipe(
        map$1(({ detail: { waveData, height } }) => ({ waveData, height })),
        share$1()
      );
    ...
    const state$ = source$.pipe(
      ...
      scan$1((acc, v) => ({ ...acc, ...v }), initialState),
      share$1()
    );

    state$.pipe(
      switchMap(state => zip(waveDataSubject, of(state, async).pipe(repeat()))),
    ...
    state$.pipe(
      startWith(initialState),
      switchMap(({ analyserInterval }) => interval$1(analyserInterval)),
    ).subscribe(_ => {
